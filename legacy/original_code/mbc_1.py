...
class DataValidator:
    ...
    def validate_csv_file(file_path: str, config: Config) -> bool:
        ...
        try:
            ...
            logging.debug(f"CSV file has {len(lines)} lines")
            logging.debug(f"First 3 lines:")
            for i, line in enumerate(lines[:3]):
                logging.debug(f"  Line {i}: '{line[:100]}...' (length: {len(line)})")
            
            for i, line in enumerate(lines):
                line_lower = line.lower()
                logging.debug(f"Checking line {i}: contains 'memberid'? {'memberid' in line_lower}, starts with '..memberid'? {line.strip().startswith('..memberid')}")
                
                # Look for line starting with "..memberid" or containing "memberid"
                if line.strip().startswith('..memberid') or 'memberid' in line_lower:
                    header_line = line
                    header_index = i
            ...
            if header_line is None:
                logging.error("Failed to find header line!")
                logging.error("Lines searched:")
            ...
            logging.debug(f"Found header at line {header_index}: {header_line[:100]}...")
            ...
            if delimiter == ' ':
                ...
                # For comma or tab delimited, use csv reader
                reader = csv.DictReader([header_line], delimiter=delimiter)
            ...
            logging.debug(f"Found headers: {sorted(headers)}")
            return True
...
class RosterProcessor:
    ...
    def process_roster(self, csv_path: str, troop_number: str) -> Dict:
        ...
        try:
            ...
            # Process CSV data with detected delimiter
            if delimiter == ' ':
                # For space-delimited files, we need special handling
                all_lines = lines[data_start_index:]
                headers = all_lines[0].strip().split()
                
                registered_adults = []
                youth_members = []
                
                for row_num, line in enumerate(all_lines[1:], start=data_start_index + 2):
                    if not line.strip():
                        continue
                ...
                # Use standard CSV processing for comma/tab delimited
                csv_reader = csv.DictReader(lines[data_start_index:], delimiter=delimiter)
                
                registered_adults = []
                youth_members = []
                
                for row_num, row in enumerate(csv_reader, start=data_start_index + 2):
                    try:
                        # Extract and clean data
                        first_name = row.get('firstname', '').strip()
                        last_name = row.get('lastname', '').strip()
...
class MBCTool:
    ...
    def process_data(self, t12_roster: str, t32_roster: str, mbc_pdfs: List[str]) -> Dict:
        ...
        try:
            ...
            raise DataProcessingError(f"Failed to process data: {e}")  # Pass through original error
