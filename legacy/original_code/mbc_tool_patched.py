...
class DataValidator:
    ...
    def validate_csv_file(file_path: str, config: Config) -> bool:
        """Validate CSV file format and required columns - NO DictReader VERSION"""
        try:
            print(f"DEBUG: *** USING COMPLETELY NEW CSV VALIDATION (NO DictReader) ***")
            ...
            print(f"DEBUG: Split into {len(lines)} lines, looking for header...")
            ...
            print(f"DEBUG: Header found, detecting delimiter...")
            ...
            print(f"DEBUG: Parsing headers manually...")
            if delimiter == ' ':
                headers = header_line.strip().split()
            else:
                headers = [h.strip() for h in header_line.split(delimiter)]
            
            print(f"DEBUG: Parsed {len(headers)} headers successfully")
            ...
            print(f"DEBUG: Required columns check...")
            print(f"DEBUG: Required: {config.REQUIRED_CSV_COLUMNS}")
            print(f"DEBUG: Found: {normalized_headers}")
            print(f"DEBUG: Missing: {missing_columns}")
...
class RosterProcessor:
    ...
    def process_roster(self, csv_path: str, troop_number: str) -> Dict:
        """Process a single roster CSV file - NO DictReader VERSION"""
        try:
