...
class MeritBadgeProcessor:
    ...
    def __init__(self):
        ...
        # Cache for scouting.org data
        self.cached_all_badges = [
            "Aerospace", "Animal Science", "Animation", "Archaeology", "Archery", 
            ...
            "Coin Collecting", "Collections", "Communication", "Composite Materials",
            "Cooking", "Crime Prevention", "Cycling", "Dentistry", "Digital Technology",
            "Disabilities Awareness", "Dog Care", "Drafting", "Drama", "Eagle Scout",
            ...
            "Metalwork", "Mining", "Model Design and Building", "Moviemaking", 
            "Music", "Nature", "Nuclear Science", "Oceanography", "Orienteering",
            "Painting", "Personal Fitness", "Personal Management", "Pets", 
            "Photography", "Pioneering", "Plant Science", "Plumbing", "Pottery", 
    ...
    def parse_merit_badge_pdfs(self, pdf_paths: List[str]) -> List[Dict]:
        """Parse Merit Badge Counselor PDF files."""
        counselors = []
        ...
        for pdf_index, pdf_path in enumerate(pdf_paths, 1):
            try:
                logger.info(f"Processing PDF {pdf_index}/{len(pdf_paths)}: {pdf_path}")
                ...
                with open(pdf_path, 'rb') as file:
                    ...
                    for page_num, page in enumerate(pdf_reader.pages):
                        try:
                            ...
                            # Parse counselor entries from the text
                            page_counselors = self._parse_counselor_text(text)
                            logger.info(f"  Found {len(page_counselors)} counselors on page {page_num + 1}")
                            counselors.extend(page_counselors)
        ...
        logger.info(f"PDF processing complete: extracted {len(counselors)} total merit badge counselors")
        ...
        if counselors:
            ...
            logger.warning("No counselors were extracted from any PDF files!")
            
        return counselors

    def _parse_counselor_text(self, text: str) -> List[Dict]:
        """Parse individual counselor entries from PDF text."""
        counselors = []
        ...
        while i < len(lines):
            ...
            expires_match = re.search(r'Expires:.*?(\d{1,2}/\d{1,2}/\d{4}).*?([A-Z][a-z]+(?: \([^)]+\))? [A-Z][a-z]+)', line)
            ...
            if expires_match:
                ...
                # Parse the name - handle format like "Christopher (Christopher) White" or "Karen MacQuilken"
                name_match = re.match(r'([A-Z][a-z]+)(?:\s+\(([^)]+)\))?\s+([A-Z][a-z]+)', full_name_section)
                
                if name_match:
                    first_name = name_match.group(1)
                    alt_first_name = name_match.group(2) if name_match.group(2) else ""
                    ...
                    # Look at next few lines for merit badges
                    j = i + 1
                    merit_badge_lines = 0
                    while j < len(lines) and merit_badge_lines < 10:  # Look for merit badges in next lines
                        next_line = lines[j]
                        ...
                        # Check if this line contains merit badges
                        # Merit badges appear as duplicated text like "Coin CollectingCoin Collecting"
                        for badge in self.all_merit_badges:
                            # Look for pattern like "CookingCooking" or just "Cooking"
                            badge_pattern = badge.replace(' ', r'\s+')  # Handle spaces in badge names
                            pattern = badge_pattern + badge_pattern + '|^' + badge_pattern + '$'
                            if re.search(pattern, next_line, re.IGNORECASE):
                                if badge not in counselor['merit_badges']:
                                    counselor['merit_badges'].append(badge)
                                    logger.debug(f"  Found merit badge: {badge}")
    ...
    def find_non_counselor_leaders(self) -> List[Dict]:
        ...
        # Check specific person for debugging
        ira_key = ('ira', 'wolf')
        if ira_key in adult_map:
            logger.info(f"IRA WOLF FOUND IN ADULT MAP: {adult_map[ira_key]}")
            if ira_key in counselor_names:
                logger.info("IRA WOLF IS IDENTIFIED AS A MERIT BADGE COUNSELOR")
            else:
                logger.info("IRA WOLF IS NOT IDENTIFIED AS A MERIT BADGE COUNSELOR")
        else:
            logger.warning("IRA WOLF NOT FOUND IN ADULT MAP")
            # Debug: show some adults to see naming patterns
            logger.info(f"SAMPLE ADULTS: {list(adult_map.keys())[:10]}")
...
<body>
    ...
    def process_all_data(self, t12_roster_path: str, t32_roster_path: str, pdf_paths: List[str]):
        ...
        # Step 3: Parse merit badge counselor PDFs
        logger.info("Step 3: Processing merit badge counselor PDFs...")
        self.merit_badge_counselors = self.parse_merit_badge_pdfs(pdf_paths)
