...
class MeritBadgeGenerator:
    ...
    def process_csv_file(self, file_path, troop_id):
        """Process CSV roster file with robust error handling"""
        try:
            ...
            self.logger.info(f"First few lines of {troop_id} CSV:")
            for i, line in enumerate(first_few_lines):
                self.logger.info(f"  Line {i+1}: {line}")
            ...
            for encoding in encodings:
                try:
                    ...
                    self.logger.info(f"Successfully parsed with encoding: {encoding}")
                    break
                except Exception as e:
            ...
            if df is None:
                # Last resort: try reading as text and manually parsing
                self.logger.warning("Standard CSV parsing failed, trying manual parsing...")
                df = self.manual_csv_parse(file_path)
            ...
            # Check for various possible column names for position
            position_cols = ['positionname', 'position', 'role', 'title']
            position_col = None
            ...
            if position_col:
                adults = df[~df[position_col].astype(str).str.contains('Youth Member', case=False, na=False)]
                self.logger.info(f"Filtered by {position_col} column")
            else:
                # If no position column, assume all are adults
                adults = df
                self.logger.warning(f"No position column found, assuming all entries are adults")
            ...
            if 'name' not in adults.columns and 'first_name' in adults.columns:
                if 'last_name' in adults.columns:
                    adults['name'] = adults['first_name'].astype(str) + ' ' + adults['last_name'].astype(str)
                else:
                    adults['name'] = adults['first_name']
            ...
            for col in text_columns:
                if col in adults.columns:
                    ...
                    adults[col] = adults[col].replace('nan', '')
                    adults[col] = adults[col].replace('NaN', '')
            ...
            adults_list = [adult for adult in adults_list if adult.get('name') and adult['name'] != 'nan']
            
            self.logger.info(f"Successfully processed {len(adults_list)} adults from {troop_id}")
            
            # Log a few sample records for verification
            if adults_list:
                self.logger.info("Sample processed records:")
                for i, adult in enumerate(adults_list[:3]):
                    self.logger.info(f"  {i+1}: {adult}")
            
            return adults_list
            
        except Exception as e:
            self.logger.error(f"Error processing CSV {file_path}: {e}")
            self.logger.error(f"Please check that the CSV file is properly formatted")
            raise
    
    def manual_csv_parse(self, file_path):
        ...
        try:
            ...
            # Parse header
            header = [col.strip().strip('"') for col in first_line.split(separator)]
            
            # Parse data rows
            for line_num, line in enumerate(lines[1:], 2):
                if line.strip():
                    try:
                        values = [val.strip().strip('"') for val in line.split(separator)]
